...
exports.Subscription.fromObject = function(obj) {
  ...
  if (obj.url[0] != "~") {
    ...
    if ("bundledVersion" in obj)
      result.bundledVersion = obj.bundledVersion;
...
class DownloadableSubscription extends RegularSubscription {
  ...
  constructor(url, title) {
    ...
    /**
     * The version of the subscription that was bundled in web extension
     * @type {string}
     * @default <code>null</code>
     */
  ...
  *serialize() {
    let {downloadStatus, lastSuccess, lastCheck, expires,
         softExpiration, errors, version, bundledVersion,
         requiredVersion, downloadCount} = this;
    ...
    if (bundledVersion)
      yield "bundledVersion=" + bundledVersion;
    if (requiredVersion)
