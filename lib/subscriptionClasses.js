...
exports.Subscription.fromURLandID = function(url, id) {
  let subscription = Subscription.knownSubscriptions.get(url);
  if (subscription)
    return subscription;

  return new NonDownloadableSubscription(url, id);
};
...
exports.Subscription.fromObject = function(obj) {
  ...
  if (obj.url[0] != "~") {
    ...
    if (!("downloadable" in obj)) {
      ...
      result = new NonDownloadableSubscription(obj.url, obj.id);
    }
  }
  else {
...
let NonDownloadableSubscription =
exports.NonDownloadableSubscription =
class NonDownloadableSubscription extends RegularSubscription {
  /**
   * Creates a `NonDownloadableSubscription` object.
   * @param {string} url The URL of the subscription.
   * @param {string} id The id of the subscription.
   * @param {string} content The content of the subscription.
   * @private
   */
  constructor(url, id) {
    super(url);
    ...
    /**
     * Sets the content of the subscription.
     * @param {string} content The content of the subscription.
     */
    this.content = null;
  }

  getDownloader(_synchronizer) {
    let content = this.content;
    return {
      download(downloadable) {
        _synchronizer._onDownloadSuccess(
          downloadable, content, null, null);
      }
    };
  }
...
class DownloadableSubscription extends RegularSubscription {
  ...
  getDownloader(_synchronizer) {
    return _synchronizer._downloader;
  }
