...
class FilterStorage {
  ...
  *subscriptions(filterText = null) {
  ...
  /**
   * Adds a user-defined filter to the storage with metadata.
   * @param {Filter|Array<Filter>} filters Filter(s) to add.
   * @param {object} metadata The metadata block.
   *
   * @return {?module:subscriptionClasses.SpecialSubscription} the subscription
   * ` the filter is in.
   */
  addFiltersWithMetadata(filters, metadata) {
    ...
    for (let filter of list) {
      if (!subscription) {
        ...
        subscription = this.addFilter(filter, subscription);
      }
    }
   ...
   *
   * @return {?Array.<object>} The array metadata if any found.
   *
   ...
   *
   * @return {?module:subscriptionClasses.SpecialSubscription} The
   *   subscription whose metadata was changed, or `undefined`
   */
   ...
   *
   * @return {?module:subscriptionClasses.SpecialSubscription} The subscription
   *   the filter is in. Either existing or added.
   */
    ...
    if (!subscription) {
      for (let currentSubscription of this._knownSubscriptions.values()) {
        if (currentSubscription instanceof SpecialSubscription &&
          ...
          return currentSubscription;   // No need to add
      }
