...
class Synchronizer {
   ...
   *   requests)
   *  @param {boolean} isMV3 MV3 won't manipulate all data
   */
  execute(subscription, manual, isMV3) {
    this._downloader.download(this._getDownloadable(subscription, manual, isMV3));
  }
  ...
  *_getDownloadables() {
  ...
  _getDownloadable(subscription, manual, isMV3 = false) {
    ...
    if (lastDownload != lastSuccess)
      ...
      if (disabled || (countable && !downloadable))
      result.method = "HEAD";
