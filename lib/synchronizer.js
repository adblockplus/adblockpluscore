...
class Synchronizer {
  ...
  execute(subscription, manual) {
    let content = subscription.content;
    let onDownloadSuccess = this._onDownloadSuccess.bind(this);
    let downloader = (subscription.updatable ?
      this._downloader :
      {
        download(downloadable) {
          onDownloadSuccess(downloadable, content, null, null);
        }
      });
