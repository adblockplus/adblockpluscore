...
function compilePatterns(filters)
{
  ...
  {
    if (filter.matchCase)
      caseSensitive += filterToRegExp(filter.pattern) + "|";
    else
      caseInsensitive += filterToRegExp(filter.pattern) + "|";
  }

  return new CompiledPatterns(
    caseSensitive ? new RegExp(caseSensitive.slice(0, -1)) : null,
    caseInsensitive ? new RegExp(caseInsensitive.slice(0, -1)) : null
  );
}
