 ...
 * let scheduler = new TimeoutScheduler();
 * downloader.onDownloadStarted = function(downloadable)
 * {
 *   console.log(`Downloading ${downloadable.url} ...`);
 * }
 * scheduler.setCallback(downloader.onSchedule.bind(downloader));
 * scheduler.start(checkInterval, initialDelay);
 *
 * @package
 */
exports.Downloader = class Downloader {
   ...
   *   on each {@link module:downloader.Downloader#onSchedule() check}.
   */
  ...
  // required to be used as `Scheduler` callback
  onSchedule() {
    this._doCheck();
  }
  ...
  getDownloadUrl(downloadable) {
    ...
    // partial subscriptons additional argument
    if (downloadable.bundledVersion) {
      url += "&webExtVersion=" +
             encodeURIComponent(downloadable.bundledVersion);
    }
...
exports.Downloadable = class Downloadable {
  ...
  constructor(url) {
    ...
    /**
     * The version of the subscription that is bundled in webext.
     * @type {string}
     */
    this.bundledVersion = null;
