...
exports.trace = makeInjector(trace, log, profile);
...
function wrapPropertyAccess(object, property, descriptor, configurable = true)
{
  ...
  if (dotIndex == -1)
  {
    ...
    let newDescriptor = Object.assign({}, descriptor, {configurable});
...
/**
 * Watches for a DOM element mutations. It logs and highlights the element
 * that was added and/or sets a breakpoint each time that happens
 *
 ...
 * Available options: **subtree** (watch the entire subtree of the element),
 * **breakpoint** (add a debugger breakpoint each time on each addition)
 *
 * @since Adblock Plus x.x
 */
function watchElement(selector, options = "") {
  let observer;
  let changeId = 0;
  ...
  const optionsChunks = options.split("+");
  const subtree = optionsChunks.indexOf("subtree") != -1;
  const breakpoint = optionsChunks.indexOf("breakpoint") != -1;

  function searchAndAttach() {
    const objects = document.querySelectorAll(selector);
    for (let object of objects) {
      if (object && object.nodeType === 1 && !object.watched) {
        for (let property of propertiesList) attach(object, property);
        object.watched = true;
        //if subtree option active, attach a listener to each parent element
        if (subtree) {
              ...
              for (let node of mutation.addedNodes) {
                if (node && node.nodeType === 1) {
                  for (let property of propertiesList)
                    attach(node, property, true);
                }
              }
            }
          }).observe(object, { childList: true, subtree: true });
        }
      }
    }
  }
  ...
  function attach(object, property, isChild = false) {
    const desc = Object.getOwnPropertyDescriptor(object, property);
    //don't attach any getter/setter where there is already one setted
    if (desc && desc.configurable === false) return;
    let { get: prevGetter, set: prevSetter } = desc || {};
    let currentValue = object[property];
    const descriptor = getDescriptor(property, prevGetter, prevSetter, isChild);
    if (descriptor === null) return;
    wrapPropertyAccess(object, property, descriptor, false);

    if (subtree || !isChild)
      for (let child of object.children) attach(child, property, true);

    function markElement(element, parent, property) {
      if (element) {
        console.groupCollapsed(`watching: ${selector} [${changeId}]`);
        log("element: ", element);
        log("added to parent element: ", parent);
        log('using the function "' + property + '"');
        console.groupEnd();
        if (element.style) {
          element.style.backgroundColor = "#dcedc1";
    ...
    function logContentChange(value, parent, property) {
      if (value) {
        console.groupCollapsed(`watching: ${selector} [${changeId}]`);
        log("the value of element: ", parent);
        log("was changed to: ", value);
        log('using the function "' + property + '"');
        console.groupEnd();
        changeId++;
      }
    ...
    function getDescriptor(property, prevGetter, prevSetter, isChild = false) {
      switch (property) {
        case "appendChild":
        case "append":
        case "insertBefore":
        case "replaceChild": {
          if (!subtree && isChild) return null;
          return {
            get() {
              return (...args) => {
                if (breakpoint) debugger;
                markElement(args[0], this, property);
                if (prevGetter) return prevGetter.call(this, ...args);
        ...
        case "insertAdjacentHTML": {
          return {
            get() {
              return (...args) => {
                if (breakpoint) debugger;
                const [position, element] = args;
                const isInsideTarget =
                  position === "afterbegin" || position === "beforeend";
                const parent = isInsideTarget ? this : this.parentElement;
                if (
                  (subtree && isChild) ||
                  (subtree && !isChild && isInsideTarget) ||
                  (!subtree && isChild && !isInsideTarget) ||
                  (!subtree && !isChild && isInsideTarget)
                ) {
                  markElement(element, parent, property);
                }
                if (prevGetter) return prevGetter.call(this, position, element);
                return currentValue.call(this, position, element);
              };
            }
          };
        }
        ...
        case "textContent": {
          ...
          return {
            set(value) {
              if (breakpoint) debugger;
              logContentChange(value, this, property);
              if (prevSetter) prevSetter.call(this, value);
...
exports["watch-element"] = makeInjector(
  watchElement,
  wrapPropertyAccess,
  log,
  profile
 ...
 *   and the perceptual hash of the image to be considered a match.
 * @param {?number} [blockBits] The block width used to generate the perceptual
 *   image hash, a number of 4 will split the image into 16 blocks
 ...
 * @returns {Function} The callback to invoke when the profile should end.
 * @private
 */
function profile(id, rate = 10)
{
  ...
  setInterval(processSamples, Math.round(60000 / Math.min(60, rate)));
