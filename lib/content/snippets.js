...
function watchElement(selector, options = "")
{
  ...
  function attach(object, property, isChild = false)
  {
    ...
    function markElement(element, parent, prop)
    {
      if (element)
      {
    ...
    function logContentChange(value, parent, prop)
    {
      if (value)
      {
        console.groupCollapsed(`watching: ${selector} [${changeId}]`);
    ...
    function getDescriptor(prop, previousGetter, previousSetter, child = false)
    {
      switch (prop)
      {
        case "appendChild":
          ...
          return {
            get()
            {
              return (...args) =>
              {
                ...
                markElement(args[0], this, prop);
                if (previousGetter) return previousGetter.call(this, ...args);
          ...
          return {
            get()
            {
              return (...args) =>
              {
                ...
                {
                  markElement(element, parent, prop);
                }
                if (previousGetter)
                  return previousGetter.call(this, position, element);
        ...
        case "textContent":
        {
          ...
          return {
            set(value)
            {
              // eslint-disable-next-line no-debugger
              if (breakpoint) debugger;
              logContentChange(value, this, prop);
              if (previousSetter) previousSetter.call(this, value);
