...
function getABPNamespace()
{
  ...
  let {ceil, floor, min, random, round} = window.Math;
  ...
  return freeze({
    Math: freeze({ceil, floor, min, random, round}),
    Object: freeze({assign, defineProperty, getOwnPropertyDescriptor, values}),
    window: freeze({getComputedStyle, setInterval, setTimeout, performance})
  });
}
