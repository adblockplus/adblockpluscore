...
function hideIfContainsImage(search, selector, searchSelector)
{
  ...
  new MutationObserver(() =>
  {
    for (let element of document.querySelectorAll(searchSelector))
    {
      ...
      if (match)
      {
        fetch(match[1]).then(response => response.arrayBuffer())
        .then(content =>
        {
...
/**
 * Prevents the execution of inline scripts.
 *
 * Only on Firefox.
 *
 ...
 *   expression.
 * @param {string} [selector] If specified, only scripts whose HTML element
 *   matches the given CSS selector are prevented from executing.
 */
function preventInlineScripts(search = null, selector = null)
{
