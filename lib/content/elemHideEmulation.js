...
let abpSelectorRegexp = /:(-abp-[\w-]+|has|has-text|xpath)\(/;
 ...
 * mutations.
 *
 * @param {Array} chain a list of selector objects that together make
 * up the pattern to match
 * @param {Node} subtree the subtree we work on
 * @param {Node[]} [targets] the nodes we are interested in. May be
 * used to optimize search.
 */
function* evaluate(chain, subtree, targets) {
  ...
  *getElements(prefix, subtree, targets) {
    ...
    if (elements) {
      for (let element of elements) {
        ...
        let iter = evaluate(
          this._innerSelectors, element, targets
        );
  ...
  setStyles(styles) {
    for (let selector of this._innerSelectors) {
...
class Pattern {
  ...
  matches(target, subtree) {
    ...
    let selectorGenerator = evaluate(
      this.selectors, subtree, targetFilter
    );
...
exports.ElemHideEmulation = class ElemHideEmulation {
  ...
  async _addSelectors(stylesheets, mutations) {
    ...
    for (let pattern of patternsToCheck) {
      ...
      let generator = evaluate(
        pattern.selectors, this.document, evaluationTargets
      );
