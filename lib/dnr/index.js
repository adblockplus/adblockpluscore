...
function asDNR(parsedFilter) {
  ...
  try {
    ...
    if (contentType & contentTypes.CSP)
      return generateCSPRules(filter, urlFilter, matchCase);
    else if (!blocking)
      return generateAllowingRules(filter, urlFilter, matchCase);
    else if (rewrite)
      return generateRedirectRules(filter, urlFilter, matchCase);

    return generateBlockingRules(filter, urlFilter, matchCase);
  }
  catch (error) {
    return new FilterParsingError("filter_invalid_regexp",
