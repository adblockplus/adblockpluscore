...
class Notifications {
  ...
  constructor() {
    ...
    /**
     * Whether to perform the blockedTotalMin/blockedTotalMax target checks. If
     * set to null (deprecated), Prefs.show_statsinpopup is considered instead.
     * Defaults to null if Prefs.show_statsinpopup exists, otherwise true.
     * @type {?boolean}
     */
    this.considerBlocked = "show_statsinpopup" in Prefs ? null : true;
  ...
  *_getDownloadables() {
  ...
  _getNextToShow() {
    ...
    let {considerBlocked} = this;
    // Compatibility for legacy adblockpluschrome
    if (considerBlocked == null)
      considerBlocked = Prefs.show_statsinpopup;
    ...
    let targetChecks = {
      ...
      platformMaxVersion:
        v => compareVersions(platformVersion, v) <= 0,
      blockedTotalMin: v => considerBlocked && Prefs.blocked_total >= v,
      blockedTotalMax: v => considerBlocked && Prefs.blocked_total <= v,
