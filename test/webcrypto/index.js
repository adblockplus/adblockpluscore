...
/**
 * @fileOverview This is a partial polyfill for crypto.subtle API, based on a
 * specialized RSA library, meant only to verify ASN.1 SHA1-based signatures.
 */
 ...
 * @returns {boolean}
 */
function verifySignature(keyData, signature, data) {
...
// Needed only in Node < 16 - borrowed from _common.js
let atob = data => Buffer.from(data, "base64").toString("binary");
let btoa = data => Buffer.from(data, "binary").toString("base64");

// avoid undesired logs in console, as if it was running in sandbox
console.warn = () => {};

// Simulate crypto.subtle utilities used to verify sitekeys.
exports.subtle = {
  importKey(_, buffer) {
    let key = btoa(String.fromCharCode(...buffer));
    return readPublicKey(key);
  },
  verify(_, keyData, sigBuffer, data) {
    if (!keyData)
      return false;

    let signature = btoa(String.fromCharCode(...sigBuffer));
    return verifySignature(keyData, signature, data);
  }
};
